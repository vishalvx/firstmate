...
fm_backend_herdr_events_capable() {  # <session>
  ...
  # In-shell substring tests, not `printf '%s' "$schema" | grep -Fq ...`: grep
  # -q closes its stdin the instant it finds a match, and $schema is ~220KB, so
  # printf routinely gets SIGPIPE'd mid-write (harmless here - grep still exits
  # 0 - but it spammed watcher.err with a misleading "write error: Broken pipe"
  # line on every single call).
  case "$schema" in
    *'events.subscribe'*) ;;
    *) return 1 ;;
  esac
