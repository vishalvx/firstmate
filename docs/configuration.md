...
FM_CRASH_THRESHOLD=10              # consecutive watcher crashes, or consecutive identical watcher wakes, allowed before daemon backoff; drives both guards
FM_CRASH_WINDOW=60                 # seconds in the crash-loop guard's sliding detection window; the repeated-wake guard counts a plain consecutive streak and does not use this
