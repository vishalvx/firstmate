...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: a project with a configured origin gets no worker until its clean task worktree matches the fetched tip of origin's resolved default branch, and any unsafe or unverifiable base stops the spawn.
A genuinely local-only project with no configured origin skips only the remote round trip: its slot is reset to the local default branch tip of the repository it is a worktree of, and the spawn refuses if that local default branch cannot be resolved; the same clean-worktree refusal still applies to it.
