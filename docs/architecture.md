...
A genuinely local-only project with no configured origin skips only the remote round trip: its slot is reset to the local default branch of the repository it is a worktree of, and launches where it stands only when no local default branch resolves; the same clean-worktree refusal still applies to it.
