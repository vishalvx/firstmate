...
`tests/fm-backend-herdr-eventwait-smoke.test.sh`, `tests/fm-transition-lib.test.sh`, `tests/fm-supervision-events.test.sh`, and the capability tests in `tests/fm-backend-herdr.test.sh` cover capability, subscribe-then-reconcile ordering, dedupe, exemptions, and polling fallback.
